...
bool ImGui::InputTextEx(const char* label, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiTextEditCallback callback, void* user_data)
{
    ...
    if (g.ActiveId == id)
    {
        ...
        if (IsKeyPressedMap(ImGuiKey_LeftArrow))                        { edit_state.OnKeyPressed(is_wordmove_key_down ? STB_TEXTEDIT_K_WORDLEFT | k_mask : STB_TEXTEDIT_K_LEFT | k_mask); }
              ...
              && !(flags & ImGuiInputTextFlags_DisableDeleting))        { edit_state.OnKeyPressed(STB_TEXTEDIT_K_DELETE | k_mask); }
        else if (IsKeyPressedMap(ImGuiKey_Backspace) && is_editable
              && !(flags & ImGuiInputTextFlags_DisableDeleting))        { edit_state.OnKeyPressed(STB_TEXTEDIT_K_BACKSPACE | k_mask); }
